...
| <a name="input_performance_insights_enabled"></a> [performance\_insights\_enabled](#input\_performance\_insights\_enabled) | Enable performance insights for RDS? This is disabled by default. A user can enable it if certain insights are needed from the Database. However, the user should ensure it is disabled once the desired outcome is achieved. | `bool` | `false` | no |
